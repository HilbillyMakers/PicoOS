...
int8_t    CANFD_WriteByteSafe                        (CANFDSPI_MODULE_ID index, uint16_t address, uint8_t   txd)
{
    ...
    // Add CRC
    crcResult = CANFD_CalculateCRC16(spiTransmitBuffer, 3);
...
int8_t    CANFD_WriteWordSafe                        (CANFDSPI_MODULE_ID index, uint16_t address, uint32_t  txd)
{
    ...
    // Add CRC
    crcResult = CANFD_CalculateCRC16(spiTransmitBuffer, 6);
...
int8_t    CANFD_ReadByteArrayWithCRC                 (CANFDSPI_MODULE_ID index, uint16_t address, uint8_t  *rxd, uint16_t nBytes, bool fromRam, bool* crcIsCorrect)
{
    ...
    crcAtController = CANFD_CalculateCRC16(spiReceiveBuffer, nBytes + 3);
...
int8_t    CANFD_WriteByteArrayWithCRC                (CANFDSPI_MODULE_ID index, uint16_t address, uint8_t  *txd, uint16_t nBytes, bool fromRam)
{
    ...
    // Add CRC
    crcResult = CANFD_CalculateCRC16(spiTransmitBuffer, spiTransferSize - 2);
...
int8_t    CANFD_Configure                            (CANFDSPI_MODULE_ID index, CAN_CONFIG* config)
{
    ...
    spiTransferError = CANFD_WriteWord(index, cREGADDR_CiCON, ciCon.word);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_OperationModeSelect                  (CANFDSPI_MODULE_ID index, CAN_OPERATION_MODE opMode)
{
    ...
    // Read
    spiTransferError = CANFD_ReadByte(index, cREGADDR_CiCON + 3, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, cREGADDR_CiCON + 3, d);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_LowPowerModeEnable                   (CANFDSPI_MODULE_ID index)
{
...
#else
    // Read
    spiTransferError = CANFD_ReadByte(index, cREGADDR_OSC, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, cREGADDR_OSC, d);
    if (spiTransferError) {
        return -2;
    }
#endif
...
CAN_OPERATION_MODE CANFD_OperationModeGet            (CANFDSPI_MODULE_ID index)
{
    ...
    // Read Opmode
    spiTransferError = CANFD_ReadByte(index, cREGADDR_CiCON + 2, &d);
    if (spiTransferError) {
...
int8_t    CANFD_TransmitChannelConfigure             (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, CAN_TX_FIFO_CONFIG* config)
{
    ...
    spiTransferError = CANFD_WriteWord(index, a, ciFifoCon.word);
...
int8_t    CANFD_TransmitChannelLoad                  (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, CAN_TX_MSGOBJ* txObj, uint8_t *txd, uint32_t txdNumBytes, bool flush)
{
    ...
    spiTransferError = CANFD_ReadWordArray(index, a, fifoReg, 3);
    if (spiTransferError) {
        return -1;
    }
    ...
    dataBytesInObject = CANFD_DlcToDataBytes((CAN_DLC) txObj->bF.ctrl.DLC);
    ...
    spiTransferError = CANFD_WriteByteArray(index, a, txBuffer, txdNumBytes + 8 + n);
    if (spiTransferError) 
    {
        return -4;
    }
    ...
    // Set UINC and TXREQ
    spiTransferError = CANFD_TransmitChannelUpdate(index, channel, flush);
    if (spiTransferError) 
    {
        return -5;
    }
...
int8_t    CANFD_TransmitChannelFlush                 (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel)
{
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, d);
...
int8_t    CANFD_TransmitChannelStatusGet             (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, CAN_TX_FIFO_STATUS* status)
{
    ...
    spiTransferError = CANFD_ReadWordArray(index, a, fifoReg, 2);
    if (spiTransferError) {
        return -1;
    }
...
{
    return CANFD_ReceiveChannelReset(index, channel);
}
/**
 * @brief 
 * 
...
int8_t    CANFD_TransmitChannelUpdate                (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, bool flush)
{
    ...
    spiTransferError = CANFD_WriteByte(index, a, ciFifoCon.byte[1]);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_TransmitRequestSet                   (CANFDSPI_MODULE_ID index, CAN_TXREQ_CHANNEL txreq)
{
    ...
    spiTransferError = CANFD_WriteWord(index, cREGADDR_CiTXREQ, w);
...
int8_t    CANFD_TransmitRequestGet                   (CANFDSPI_MODULE_ID index, uint32_t* txreq)
{
    ...
    spiTransferError = CANFD_ReadWord(index, cREGADDR_CiTXREQ, txreq);
...
int8_t    CANFD_FilterObjectConfigure                (CANFDSPI_MODULE_ID index, CAN_FILTER filter, CAN_FILTEROBJ_ID* id)
{
    ...
    spiTransferError = CANFD_WriteWord(index, a, fObj.word);
...
int8_t    CANFD_FilterMaskConfigure                  (CANFDSPI_MODULE_ID index, CAN_FILTER filter, CAN_MASKOBJ_ID* mask)
{
    ...
    spiTransferError = CANFD_WriteWord(index, a, mObj.word);
...
int8_t    CANFD_FilterToFifoLink                     (CANFDSPI_MODULE_ID index, CAN_FILTER filter, CAN_FIFO_CHANNEL channel, bool enable)
{
    ...
    spiTransferError = CANFD_WriteByte(index, a, fCtrl.byte);
...
int8_t    CANFD_FilterEnable                         (CANFDSPI_MODULE_ID index, CAN_FILTER filter)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &fCtrl.byte);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_DeviceNetFilterCountSet              (CANFDSPI_MODULE_ID index, CAN_DNET_FILTER_SIZE dnfc)
{
    ...
    // Read CiCON byte 0
    spiTransferError = CANFD_ReadByte(index, cREGADDR_CiCON, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, cREGADDR_CiCON, d);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_ReceiveChannelStatusGet              (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, CAN_RX_FIFO_STATUS* status)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &ciFifoSta.byte[0]);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_ReceiveMessageGet                    (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, CAN_RX_MSGOBJ* rxObj, uint8_t *rxd, uint8_t nBytes)
{
    ...
    spiTransferError = CANFD_ReadByteArray(index, a, ba, n);
    if (spiTransferError) {
        return -3;
    }
    ...
    // UINC channel
    spiTransferError = CANFD_ReceiveChannelUpdate(index, channel);
    if (spiTransferError) {
        return -4;
    }
...
int8_t    CANFD_TefStatusGet                         (CANFDSPI_MODULE_ID index, CAN_TEF_FIFO_STATUS* status)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &ciTefSta.byte[0]);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_TefMessageGet                        (CANFDSPI_MODULE_ID index, CAN_TEF_MSGOBJ* tefObj)
{
    ...
    // Set UINC
    spiTransferError = CANFD_TefUpdate(index);
    if (spiTransferError) {
        return -3;
    }
...
int8_t    CANFD_TefReset                             (CANFDSPI_MODULE_ID index)
{
    ...
    // Write byte
    spiTransferError = CANFD_WriteByte(index, a, ciTefCon.byte[1]);
...
int8_t    CANFD_TefConfigure                         (CANFDSPI_MODULE_ID index, CAN_TEF_CONFIG* config)
{
    ...
    spiTransferError = CANFD_WriteWord(index, cREGADDR_CiTEFCON, ciTefCon.word);
...
int8_t    CANFD_ModuleEventGet                       (CANFDSPI_MODULE_ID index, CAN_MODULE_EVENT* flags)
{
    ...
    spiTransferError = CANFD_ReadHalfWord(index, cREGADDR_CiINTFLAG, &intFlags.word);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_ModuleEventEnable                    (CANFDSPI_MODULE_ID index, CAN_MODULE_EVENT flags)
{
    ...
    spiTransferError = CANFD_ReadHalfWord(index, a, &intEnables.word);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteHalfWord(index, a, intEnables.word);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_ModuleEventClear                     (CANFDSPI_MODULE_ID index, CAN_MODULE_EVENT flags)
{
    ...
    // Write
    spiTransferError = CANFD_WriteHalfWord(index, a, intFlags.word);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_ModuleEventRxCodeGet                 (CANFDSPI_MODULE_ID index, CAN_RXCODE* rxCode)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &rxCodeByte);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_ModuleEventTxCodeGet                 (CANFDSPI_MODULE_ID index, CAN_TXCODE* txCode)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &txCodeByte);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_ModuleEventFilterHitGet              (CANFDSPI_MODULE_ID index, CAN_FILTER* filterHit)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &filterHitByte);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_ModuleEventIcodeGet                  (CANFDSPI_MODULE_ID index, CAN_ICODE* icode)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &icodeByte);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_TransmitEventGet                     (CANFDSPI_MODULE_ID index, uint32_t* txif)
{
    ...
    spiTransferError = CANFD_ReadWord(index, cREGADDR_CiTXIF, txif);
...
int8_t    CANFD_TransmitEventAttemptGet              (CANFDSPI_MODULE_ID index, uint32_t* txatif)
{
    ...
    spiTransferError = CANFD_ReadWord(index, cREGADDR_CiTXATIF, txatif);
...
int8_t    CANFD_TransmitChannelIndexGet              (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, uint8_t* idx)
{
    ...
    spiTransferError = CANFD_ReadWord(index, a, &ciFifoSta.word);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_TransmitChannelEventEnable           (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, CAN_TX_FIFO_EVENT  flags)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &ciFifoCon.byte[0]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, ciFifoCon.byte[0]);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_TransmitChannelEventAttemptClear     (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel)
{
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, ciFifoSta.byte[0]);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_ReceiveEventGet                      (CANFDSPI_MODULE_ID index, uint32_t* rxif)
{
    ...
    spiTransferError = CANFD_ReadWord(index, cREGADDR_CiRXIF, rxif);
...
int8_t    CANFD_ReceiveEventOverflowGet              (CANFDSPI_MODULE_ID index, uint32_t* rxovif)
{
    ...
    spiTransferError = CANFD_ReadWord(index, cREGADDR_CiRXOVIF, rxovif);
...
{
    return CANFD_TransmitChannelIndexGet(index, channel, idx);
}
/**
 * @brief 
 * 
...
int8_t    CANFD_TefEventEnable                       (CANFDSPI_MODULE_ID index, CAN_TEF_FIFO_EVENT  flags)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &ciTefCon.byte[0]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, ciTefCon.byte[0]);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_TefEventOverflowClear                (CANFDSPI_MODULE_ID index)
{
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, ciTefSta.byte[0]);
    if (spiTransferError) 
    {
        return -2;
    }
...
int8_t    CANFD_ErrorCountTransmitGet                (CANFDSPI_MODULE_ID index, uint8_t* tec)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, tec);
...
int8_t    CANFD_ErrorCountReceiveGet                 (CANFDSPI_MODULE_ID index, uint8_t* rec)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, rec);
...
int8_t    CANFD_ErrorStateGet                        (CANFDSPI_MODULE_ID index, CAN_ERROR_STATE* flags)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &f);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_ErrorCountStateGet                   (CANFDSPI_MODULE_ID index, uint8_t* tec, uint8_t* rec, CAN_ERROR_STATE* flags)
{
    ...
    spiTransferError = CANFD_ReadWord(index, a, &ciTrec.word);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_BusDiagnosticsGet                    (CANFDSPI_MODULE_ID index, CAN_BUS_DIAGNOSTIC* bd)
{
    ...
    spiTransferError = CANFD_ReadWordArray(index, a, w, 2);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_BusDiagnosticsClear                  (CANFDSPI_MODULE_ID index)
{
    ...
    spiTransferError = CANFD_WriteWordArray(index, a, w, 2);
...
int8_t    CANFD_EccEnable                            (CANFDSPI_MODULE_ID index)
{
    ...
    // Read
    spiTransferError = CANFD_ReadByte(index, cREGADDR_ECCCON, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, cREGADDR_ECCCON, d);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_EccEventGet                          (CANFDSPI_MODULE_ID index, CAN_ECC_EVENT* flags)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &eccStatus);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_EccParitySet                         (CANFDSPI_MODULE_ID index, uint8_t parity)
{
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, cREGADDR_ECCCON + 1, parity);
...
int8_t    CANFD_EccParityGet                         (CANFDSPI_MODULE_ID index, uint8_t* parity)
{
    ...
    // Read
    spiTransferError = CANFD_ReadByte(index, cREGADDR_ECCCON + 1, parity);
...
int8_t    CANFD_EccErrorAddressGet                   (CANFDSPI_MODULE_ID index, uint16_t* a)
{
    ...
    // Read
    spiTransferError = CANFD_ReadWord(index, cREGADDR_ECCSTA, &reg.word);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_EccEventEnable                       (CANFDSPI_MODULE_ID index, CAN_ECC_EVENT flags)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &eccInterrupts);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, eccInterrupts);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_EccEventClear                        (CANFDSPI_MODULE_ID index, CAN_ECC_EVENT flags)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &eccStat);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, eccStat);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_CrcEventEnable                       (CANFDSPI_MODULE_ID index, CAN_CRC_EVENT  flags)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &crc);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, crc);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_CrcValueGet                          (CANFDSPI_MODULE_ID index, uint16_t* crc)
{
    ...
    spiTransferError = CANFD_ReadHalfWord(index, cREGADDR_CRC, crc);
...
int8_t    CANFD_RamInit                              (CANFDSPI_MODULE_ID index, uint8_t d)
{
    ...
    for (k = 0; k < ((cRAM_SIZE / SPI_DEFAULT_BUFFER_LENGTH) * 2); k++) {
        spiTransferError = CANFD_WriteByteArray(index, a, txd, SPI_DEFAULT_BUFFER_LENGTH/2);
        if (spiTransferError) {
            return -1;
        }
...
int8_t    CANFD_TimeStampEnable                      (CANFDSPI_MODULE_ID index)
{
    ...
    // Read
    spiTransferError = CANFD_ReadByte(index, cREGADDR_CiTSCON + 2, &d);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, cREGADDR_CiTSCON + 2, d);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_TimeStampGet                         (CANFDSPI_MODULE_ID index, uint32_t* ts)
{
    ...
    // Read
    spiTransferError = CANFD_ReadWord(index, cREGADDR_CiTBC, ts);
...
int8_t    CANFD_TimeStampSet                         (CANFDSPI_MODULE_ID index, uint32_t  ts)
{
    ...
    // Write
    spiTransferError = CANFD_WriteWord(index, cREGADDR_CiTBC, ts);
...
int8_t    CANFD_TimeStampPrescalerSet                (CANFDSPI_MODULE_ID index, uint16_t ps)
{
    ...
    // Write
    spiTransferError = CANFD_WriteHalfWord(index, cREGADDR_CiTSCON, ps);
...
/*****************************************************************************
 *****************************************************************************
 Section: Section: Oscillator and Bit Time */
...
int8_t    CANFD_OscillatorControlSet                 (CANFDSPI_MODULE_ID index, CAN_OSC_CTRL  ctrl)
{
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, cREGADDR_OSC, osc.byte[0]);
...
int8_t    CANFD_OscillatorStatusGet                  (CANFDSPI_MODULE_ID index, CAN_OSC_STATUS* status)
{
    ...
    // Read
    spiTransferError = CANFD_ReadByte(index, cREGADDR_OSC + 1, &osc.byte[1]);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_BitTimeConfigure                     (CANFDSPI_MODULE_ID index, CAN_BITTIME_SETUP bitTime, CAN_SSP_MODE sspMode, CAN_SYSCLK_SPEED clk)
{
    ...
    switch (clk) {
        case CAN_SYSCLK_40M:
            spiTransferError = CANFD_BitTimeConfigureNominal40MHz(index, bitTime);
            if (spiTransferError) return spiTransferError;

            spiTransferError = CANFD_BitTimeConfigureData40MHz(index, bitTime, sspMode);
            break;
        case CAN_SYSCLK_20M:
            spiTransferError = CANFD_BitTimeConfigureNominal20MHz(index, bitTime);
            if (spiTransferError) return spiTransferError;

            spiTransferError = CANFD_BitTimeConfigureData20MHz(index, bitTime, sspMode);
            break;
        case CAN_SYSCLK_10M:
            spiTransferError = CANFD_BitTimeConfigureNominal10MHz(index, bitTime);
            if (spiTransferError) return spiTransferError;

            spiTransferError = CANFD_BitTimeConfigureData10MHz(index, bitTime, sspMode);
            break;
        default:
            spiTransferError = -1;
            break;
...
int8_t    CANFD_BitTimeConfigureNominal40MHz         (CANFDSPI_MODULE_ID index, CAN_BITTIME_SETUP bitTime)
{
    ...
    spiTransferError = CANFD_WriteWord(index, cREGADDR_CiNBTCFG, ciNbtcfg.word);
...
int8_t    CANFD_BitTimeConfigureData40MHz            (CANFDSPI_MODULE_ID index, CAN_BITTIME_SETUP bitTime, CAN_SSP_MODE sspMode)
{
    ...
    // Write Bit time registers
    spiTransferError = CANFD_WriteWord(index, cREGADDR_CiDBTCFG, ciDbtcfg.word);
    if (spiTransferError) {
        return -2;
    }
    ...
    spiTransferError = CANFD_WriteWord(index, cREGADDR_CiTDC, ciTdc.word);
    if (spiTransferError) {
        return -3;
    }
...
int8_t    CANFD_GpioModeConfigure                    (CANFDSPI_MODULE_ID index, GPIO_PIN_MODE gpio0, GPIO_PIN_MODE gpio1)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &iocon.byte[3]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, iocon.byte[3]);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_GpioDirectionConfigure               (CANFDSPI_MODULE_ID index, GPIO_PIN_DIRECTION gpio0, GPIO_PIN_DIRECTION gpio1)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &iocon.byte[0]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, iocon.byte[0]);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_GpioPinSet                           (CANFDSPI_MODULE_ID index, GPIO_PIN_POS pos, GPIO_PIN_STATE latch)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &iocon.byte[1]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = CANFD_WriteByte(index, a, iocon.byte[1]);
    if (spiTransferError) {
        return -2;
    }
...
int8_t    CANFD_GpioPinRead                          (CANFDSPI_MODULE_ID index, GPIO_PIN_POS pos, GPIO_PIN_STATE* state)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &iocon.byte[2]);
    if (spiTransferError) {
        return -1;
    }
...
int8_t    CANFD_FifoIndexGet                         (CANFDSPI_MODULE_ID index, CAN_FIFO_CHANNEL channel, uint8_t* mi)
{
    ...
    spiTransferError = CANFD_ReadByte(index, a, &b);
    if (spiTransferError) {
        return -1;
    }
